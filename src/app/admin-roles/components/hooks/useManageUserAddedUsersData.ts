...
export default function useManageUserAddedUsersData(adminRoleId: number) {
    const {
    ...
    const { data, error } = useSuspenseQuery({
        ...
        queryFn: () => searchAdminUsersInAdminRole(adminRoleId, keyword, 1000, 1)
    });
    ...
    const removeUsersMutation = useMutation<string, unknown, number[]>({
        mutationFn: (adminUserIds) => removeUsersFromAdminRole(adminRoleId, adminUserIds),
        onSuccess: () => {
